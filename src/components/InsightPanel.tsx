      ...
      <section>
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-lg font-bold tracking-normal">5초 해석</h2>
          </div>
            ...
            <div className="space-y-2">
              {insights.map((insight) => {
                ...
                return (
                  <article
                    key={insight.id}
                    className={`rounded-2xl border bg-card/40 p-3 ${severityBorderClasses[insight.severity]}`}
                  >
                    <div className="flex items-start gap-2.5">
                      <div
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${severityClasses[insight.severity]}`}
                      >
                        <Icon className="h-4 w-4" aria-hidden="true" />
                      </div>
                      <div className="min-w-0">
                        <h3 className="text-safe text-sm font-bold">{insight.title}</h3>
                        ...
                        {badges.length > 0 ? (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {badges.map((tag) => (
                              <span
                                key={`${insight.id}-${tag}`}
                                className="rounded-full bg-muted/70 px-2 py-0.5 text-[0.65rem] font-bold text-muted-foreground"
                              >
                                {tag}
                              </span>
                            ))}
